...
const ELECTION_META = [
  { year: '2022', votes: '≈22,845', won: true },
  { year: '2025', votes: '22,114', won: false },
] as const
    ...
    <section id="stats" className="bg-navy text-white" aria-labelledby="stats-heading">
      <div className="section-pad">
        <div className="container-site section-y">
          <Reveal>
            <p className="micro-label text-gold">{t.stats.eyebrow}</p>
            <h2
              id="stats-heading"
          ...
          <div className="mt-12 grid gap-6 md:grid-cols-2">
            {t.elections.map((election, i) => (
              <Reveal key={ELECTION_META[i].year} delay={0.05}>
                <div
                  className={`flex h-full flex-col justify-between gap-6 border-b-4 p-7 sm:p-9 ${
                    ELECTION_META[i].won ? 'border-gold bg-blue/40' : 'border-red/60 bg-white/[0.04]'
                  }`}
                >
                  <div className="flex items-start justify-between gap-6">
                    <div>
                      <p className="font-display text-7xl leading-none tabular text-gold sm:text-8xl">
                        {ELECTION_META[i].votes}
                      </p>
                      <p className="micro-label mt-3 text-white/60">{t.stats.votesLabel}</p>
                    </div>
                    <span className="font-display text-4xl uppercase leading-none text-white/25">
                      {ELECTION_META[i].year}
                    </span>
                  </div>
                  <div>
                    ...
                    <p
                      className={`mt-4 inline-flex items-center gap-2 px-3 py-1.5 micro-label ${
                        ELECTION_META[i].won ? 'bg-gold text-navy' : 'bg-red text-white'
                      }`}
                    >
          ...
          <p className="mt-8 max-w-xl text-sm text-white/40">{t.stats.disclaimer}</p>
        </div>
      </div>
    </section>
  )
